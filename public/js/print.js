  ...
  try {
    if (type === 'transactions') {
      ...
      if (!response.data || response.data.length === 0) {
        content.innerHTML = '<div class="status-msg">No transactions found.</div>';
        return;
      }
      ...
      response.data.forEach(txn => {
        const typeStyle = txn.type === 'income' ? 'color: #006600' : 'color: #cc0000';
        html += `
          <tr>
      ...
      content.innerHTML = '<div class="status-msg">Invalid report type specified.</div>';
      return;
    }
