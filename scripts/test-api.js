...
const run = async () => {
  ...
  try {
    ...
    console.log(
      'GET /api/categories =>',
      categoriesBefore.status,
      categoriesBefore.body.data.length,
      'categories'
    );
    ...
    console.log(
      'POST /api/categories =>',
      newCategory.status,
      newCategory.body.data
    );

    const updateCategory = await request(
      `/api/categories/${newCategory.body.data.id}`,
      {
        method: 'PUT',
    ...
    console.log(
      ...
      createTransaction.body.data
    );

    const updateTransaction = await request(
      `/api/transactions/${createTransaction.body.data.id}`,
      {
        method: 'PUT',
        body: {
          category_id: newCategory.body.data.id,
    ...
    console.log(
      ...
      transactions.status,
      transactions.body.data.length,
      'transactions'
    );

    const deleteTransaction = await request(
      `/api/transactions/${createTransaction.body.data.id}`,
      {
        method: 'DELETE',
      }
