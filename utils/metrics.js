...
const emitGenerated = (reportType, labels) => {
  ...
  logger.info({ metric: 'reports_generated_total', reportType, labels }, 'metric_emitted');
};

const emitFailed = (reportType, labels, attempts) => {
  failedCounter.inc({ reportType, ...labels, attempts: String(attempts) }, 1);
  logger.info({ metric: 'reports_failed_total', reportType, labels, attempts }, 'metric_emitted');
};

const emitSkipped = (reportType, labels) => {
  skippedCounter.inc({ reportType, ...labels }, 1);
  logger.info({ metric: 'reports_skipped_total', reportType, labels }, 'metric_emitted');
};
...
const emitLockAcquired = (reportType) => {
  ...
  logger.info({ metric: 'scheduler_lock_acquired_total', reportType }, 'metric_emitted');
};

const emitLockFailed = (reportType) => {
  lockFailedCounter.inc({ reportType });
  logger.info({ metric: 'scheduler_lock_failed_total', reportType }, 'metric_emitted');
};

const emitAuditEvent = (action) => {
  auditEventCounter.inc({ action });
  logger.info({ metric: 'report_audit_events_total', action }, 'metric_emitted');
};
...
const recordLastReportTimestamp = (reportType, timestamp) => {
  ...
  logger.info({ metric: 'report_last_generated_timestamp_seconds', reportType, seconds }, 'metric_emitted');
};
